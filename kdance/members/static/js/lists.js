...
function populateSecondSelect(previousValue) {
  ...
  switch (previousValue) {
    case '1':
      ...
      $('#menu-3-select').attr('hidden', false);
      for (let [key, value] of Object.entries(LIST_INFO_MAPPING)) {
        $('#menu-3-select').append($('<option>', { value: key, text: value, selected: key == '0' }));
      }
  }
}
...
function onMainChange(mainValue) {
  ...
  if (mainValue == '0') {
    ...
    $('#menu-3-select').attr('hidden', true);
  } else {
...
function getMembersPerCourse() {
  ...
  $.ajax({
    ...
    success: (data) => {
      ...
      if (document.querySelector('#data-table').className === '') {
        ...
        $('#data-table').bootstrapTable('load', data.map((m) => {
          return {
            ...m,
            courses: m.courses.map((c) => c.name),
          }
